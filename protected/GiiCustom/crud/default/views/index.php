...
?>
<?= '<?php' ?> Pjax::begin(['id' => 'gridData<?= Inflector::camel2id(StringHelper::basename($generator->modelClass)) ?>', 'timeout' => false, 'enablePushState' => true, 'enableReplaceState' => false]); ?>
<div class="card">
  <div class="card-header d-flex">
    <div class="header-title flex-grow-1">
      <h4 class="card-title"><?= Inflector::camel2words(StringHelper::basename($generator->modelClass)) ?></h4>
    </div>
    <div style="width:20%">
    ...
    <div class="ms-3">
      <?= "<?php // echo" ?>Html::a('<i class="fa fa-search"></i> Cari', '#<?= Inflector::camel2id(StringHelper::basename($generator->modelClass)) ?>-search', ['class' => 'btn btn-sm btn-warning', 'data-bs-toggle' => 'collapse' , 'aria-controls' => '<?= Inflector::camel2id(StringHelper::basename($generator->modelClass)) ?>-search']); ?>
      <?= "<?= " ?> Html::button('Tambah |
      ...
      </svg>', ['value' => Url::to(['create']), 'title' => 'Form '.<?= $generator->generateString(Inflector::camel2words(StringHelper::basename($generator->modelClass))) ?>, 'class' => 'showModalButton btn btn-primary']); ?>
    </div>
  </div>
  <div class="card-body">
    <div class="<?= Inflector::camel2id(StringHelper::basename($generator->modelClass)) ?>-index col-lg-12 p-0 table-responsive">
      ...
      <?php if ($generator->indexWidgetType === 'grid') : ?>
        ...
        ','#', ['value' => Url::to(['view', <?= $generator->generateUrlParams() ?>]), 'title' => 'Detail '.<?= $generator->generateString(Inflector::camel2words(StringHelper::basename($generator->modelClass))) ?>, 'class' => 'showModalButton text-primary me-md-1']).
        Html::a('
        <svg class="icon-16" width="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        ','#', ['value' => Url::to(['update', <?= $generator->generateUrlParams() ?>]), 'title' => 'Form Edit '.<?= $generator->generateString(Inflector::camel2words(StringHelper::basename($generator->modelClass))) ?>, 'class' => 'showModalButton text-success me-md-1']).
        Html::a('
        <svg class="icon-16" width="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        ','#',['<?= $generator->modelClass::primaryKey()[0] ?>' => $model-><?= $generator->modelClass::primaryKey()[0] ?>, 'title' => 'Hapus Data?', 'class' => 'text-danger delete']);
        }
        ],
        ],
        ]); ?>
